...
@triton.jit
def sym_constrain_range_kernel(x):
    # Placeholder kernel: sym_constrain_range is a symbolic-shape compiler hint
    # that validates a scalar bound and performs no tensor computation, so there
    # is nothing to compute on device. Kept for parity with the sym_* op family
    # (sym_stride, sym_storage_offset).
    return x
